...
class Advanced_Visit_Counter_Public {
	...
	public function public_avc_visit_counter( $atts = [], $content = null, $tag = '' ) {
		...
		if( $atts['type'] !== 'individual' && $atts['type'] !== 'global' && $atts['type'] !== 'customized'){
			if( is_admin() ) {
				$article_id = 1;	
			} else {
				$article_id = '';
		...
		if( $atts['today'] == "true" ) {
			$TodaypageCnt = $wpdb->get_var("SELECT COUNT(*) FROM {$tbl_history} WHERE article_id=$articleID AND DATE(`date`) = CURDATE()");
			$todaysCount = " | Today: ".$TodaypageCnt;
		} else {
			$todaysCount = "";
		}
		...
		if( $atts['global'] == "true" ) {
			...
			$allTimeCount = " | Total: ".($allTime+$existingAllCounts);
		} else {
			$allTimeCount = "";
		}

		if( !empty($atts['width']) ) {
			$shWidth = " width:".$atts['width']."px;";
		} else {
			$shWidth = "";
		}

		echo $html = "<div class='avc_visit_counter_front' style='".$borderCSS.$bgColor.$borderRadius.$font_size.$font_style.$font_color."".$shWidth."'>".__($counter_label)." ".$pageCnt." ".$todaysCount." ".$allTimeCount."</div>";
	...
	public function public_add_counter_to_content( $content ){
		...
		if( is_array($avcConfig->post_types) && in_array($currentPostType, $avcConfig->post_types) && ! is_feed() && ! is_home() && !in_array($article_id,$exShowCnt) && $this->AVP_isWooCommercePage() != 1 ){
			...
			$widget_width = $avcConfig->widget_width;

			if( !empty($widget_width) ) {
				$widget_width = " width: ".$widget_width."px;";
			} else {
				$widget_width = " width: auto;";
			}

			if( $avcConfig->show_today_count == "true" ) {
				$TodaypageCnt = $wpdb->get_var("SELECT COUNT(*) FROM {$tbl_history} WHERE article_id=$articleID AND DATE(`date`) = CURDATE()");
				$todaysCount = " | Today: ".$TodaypageCnt;
			} else {
				$todaysCount = "";
			...
			if( $avcConfig->show_global_count == "true" ) {
				$allTime = $wpdb->get_var("SELECT COUNT(*) FROM {$tbl_history}");
				$allTimeCount = " | Total: ".($allTime+$existingAllCounts);
			} else {
				$allTimeCount = "";
			...
			$style = 'style="'.$widget_width.' border: '.$avc_config->apv_default_border_width.'px solid '.$avc_config->apv_default_border_color.'; color:'.$avc_config->avc_default_text_color_of_counter.'; background-color:'.$bgColorBox.'; border-radius: '.$avc_config->apv_default_border_radius.'px; '.$widAlignmentCss.'"';
			...
			$ShortcodeHtml = "<div class='avc_visit_counter_front_simple' ".$style.">".__("".$label." ").$pageCnt." ".$todaysCount." ".$allTimeCount."</div>";
			$pType = get_post_type();
